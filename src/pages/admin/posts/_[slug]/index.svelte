<script>
  import QuillEditor from "../../../write/_components/QuillEditor.svelte";
  import QuillToolbar from "../../../write/_components/QuillToolbar.svelte";
  import { currentDocument, user } from "../../../../stores";
  ...
  function updateDocumentStore(content) {
  ...
  async function saveDocumentToDB() {
    let params = {
      document: {
        ...
        sections_attributes: $currentDocument.sections,
      },
    };
    const data = await Api[`${$currentDocument.id ? "patch" : "post"}`](
  ...
  async function fetchPost() {
    const res = await fetch(`${envURL}/posts/${$params.slug}`);
